...
class SK_API AlRefCntBase {
public:
    /** Default construct, initializing the reference count to 1.
    */
    AlRefCntBase() : fRefCnt(1) {}

    /** Destruct, asserting that the reference count is 1.
    */
    virtual ~AlRefCntBase() {
    #ifdef __AL_DEBUG__
...
private:
    ...
    // and conditionally call AlRefCnt::internal_dispose().
    friend class SkWeakRefCnt;

    mutable std::atomic<int32_t> fRefCnt;

    AlRefCntBase(AlRefCntBase&&) = delete;
    AlRefCntBase(const AlRefCntBase&) = delete;
    AlRefCntBase& operator=(AlRefCntBase&&) = delete;
    AlRefCntBase& operator=(const AlRefCntBase&) = delete;
};

#ifdef SK_REF_CNT_MIXIN_INCLUDE
// It is the responsibility of the following include to define the type AlRefCnt.
// This AlRefCnt should normally derive from SkRefCntBase.
#include SK_REF_CNT_MIXIN_INCLUDE
#else
class SK_API AlRefCnt : public AlRefCntBase {
...
// This is a variant of AlRefCnt that's Not Virtual, so weighs 4 bytes instead of 8 or 16.
// There's only benefit to using this if the deriving class does not otherwise need a vtable.
template <typename Derived>
class AlNVRefCnt {
public:
    AlNVRefCnt() : fRefCnt(1) {}
    ~AlNVRefCnt() {
    #ifdef __AL_DEBUG__
...
private:
    ...
    AlNVRefCnt(AlNVRefCnt&&) = delete;
    AlNVRefCnt(const AlNVRefCnt&) = delete;
    AlNVRefCnt& operator=(AlNVRefCnt&&) = delete;
    AlNVRefCnt& operator=(const AlNVRefCnt&) = delete;
};
 ...
 *
 *  This can be used for classes inheriting from AlRefCnt, but it also works for other
 *  classes that match the interface, but have different internal choices: e.g. the hosted class
 *  may have its ref/unref be thread-safe, but that is not assumed/imposed by sk_sp.
 */
template <typename T> class al_sp {
public:
    using element_type = T;

    constexpr al_sp() : fPtr(nullptr) {}
    constexpr al_sp(std::nullptr_t) : fPtr(nullptr) {}

    /**
     *  Shares the underlying object by calling ref(), so that both the argument and the newly
     *  created sk_sp both have a reference to it.
     */
    al_sp(const al_sp<T>& that) : fPtr(AlSafeRef(that.get())) {}
    template <typename U,
              typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
    al_sp(const al_sp<U>& that) : fPtr(AlSafeRef(that.get())) {}
     ...
     */
    al_sp(al_sp<T>&& that) : fPtr(that.release()) {}
    template <typename U,
              typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
    al_sp(al_sp<U>&& that) : fPtr(that.release()) {}

    /**
     *  Adopt the bare pointer into the newly created sk_sp.
     *  No call to ref() or unref() will be made.
     */
    explicit al_sp(T* obj) : fPtr(obj) {}
    ...
    al_sp<T>& operator=(std::nullptr_t) { this->reset(); return *this; }
     ...
     *  object.
     */
    al_sp<T>& operator=(const al_sp<T>& that) {
        if (this != &that) {
    ...
    template <typename U,
              typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
    al_sp<T>& operator=(const al_sp<U>& that) {
        this->reset(SkSafeRef(that.get()));
        return *this;
    }
     ...
     *  will be made.
     */
    al_sp<T>& operator=(al_sp<T>&& that) {
        this->reset(that.release());
        return *this;
    }
    template <typename U,
              typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
    al_sp<T>& operator=(al_sp<U>&& that) {
        this->reset(that.release());
        return *this;
    }
    ...
    void swap(al_sp<T>& that) /*noexcept*/ {
        using std::swap;
        swap(fPtr, that.fPtr);
    }
...
template <typename T> inline void swap(al_sp<T>& a, al_sp<T>& b) /*noexcept*/ {
    a.swap(b);
}

template <typename T, typename U> inline bool operator==(const al_sp<T>& a, const al_sp<U>& b) {
    return a.get() == b.get();
}
template <typename T> inline bool operator==(const al_sp<T>& a, std::nullptr_t) /*noexcept*/ {
    return !a;
}
template <typename T> inline bool operator==(std::nullptr_t, const al_sp<T>& b) /*noexcept*/ {
    return !b;
}
...
auto operator<<(std::basic_ostream<C, CT>& os, const al_sp<T>& sp) -> decltype(os << sp.get()) {
    return os << sp.get();
}

template <typename T, typename... Args>
al_sp<T> sk_make_sp(Args&&... args) {
    return al_sp<T>(new T(std::forward<Args>(args)...));
}
 ...
 */
template <typename T> al_sp<T> sk_ref_sp(T* obj) {
    return al_sp<T>(SkSafeRef(obj));
}

template <typename T> al_sp<T> sk_ref_sp(const T* obj) {
    return al_sp<T>(const_cast<T*>(SkSafeRef(obj)));
}
