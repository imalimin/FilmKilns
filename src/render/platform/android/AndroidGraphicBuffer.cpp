...
public:
    ...
    typedef EGLImageKHR (*pfnCreateImageKHR)(EGLDisplay dpy, EGLContext ctx, EGLenum target,
    ...
    bool EnsureInitialized() {
        ...
        fImageTargetTexture2DOES = (pfnImageTargetTexture2DOES) dlsym(handle,
        ...
        fGraphicBufferCtor = (pfnGraphicBufferCtor) dlsym(handle,
        ...
        fGraphicBufferLockRect = (pfnGraphicBufferLockRect) dlsym(handle,
        ...
        fGraphicBufferReallocate = (pfnGraphicBufferReallocate) dlsym(handle,
...
namespace mozilla {
    ...
    AndroidGraphicBuffer::AndroidGraphicBuffer(uint32_t width, uint32_t height, uint32_t usage,
            ...
            mWidth(width), mHeight(height), mUsage(usage), mFormat(format), mHandle(0),
            mEGLImage(0) {
    }
    ...
    AndroidGraphicBuffer::EnsureBufferCreated() {
        if (!mHandle) {
            ...
            sGLFunctions.fGraphicBufferCtor(mHandle, mWidth, mHeight, GetAndroidFormat(mFormat),
                                            GetAndroidUsage(mUsage));
        }
    ...
    AndroidGraphicBuffer::Reallocate(uint32_t aWidth, uint32_t aHeight, gfxImageFormat aFormat) {
        ...
        if (!gTryRealloc || sGLFunctions.fGraphicBufferReallocate(mHandle, aWidth, aHeight,
                                                                  GetAndroidFormat(aFormat)) != 0) {
            DestroyBuffer();
            EnsureBufferCreated();
    ...
    AndroidGraphicBuffer::EnsureEGLImage() {
        ...
        mEGLImage = sGLFunctions.fCreateImageKHR(sGLFunctions.fGetDisplay(EGL_DEFAULT_DISPLAY),
                                                 EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID,
                                                 (EGLClientBuffer) nativeBuffer, eglImgAttrs);
